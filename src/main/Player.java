...
public class Player implements AudioProcessor {
	...
	public Player(AudioProcessor beforeWSOLAProcessor,AudioProcessor afterWSOLAProcessor,Double factor){
	...
	public void load(File file) {
		...
		durationInSeconds = fileFormat.getFrameLength() / (format.getFrameRate()*format.getChannels());
		pauzedAt = 0;
		currentTime = 0;
	...
	public void play(double startTime) {
		if(state == PlayerState.NO_FILE_LOADED){
			...
			try {
				...
				wsola = new WaveformSimilarityBasedOverlapAdd(Parameters.automaticDefaults(tempo,format.getSampleRate()*format.getChannels()));
				audioPlayer = new AudioPlayer(format,wsola.getInputBufferSize());
				if(format.getChannels() != 1){
					dispatcher = AudioDispatcherFactory.fromFile(loadedFile,wsola.getInputBufferSize() * format.getChannels(),wsola.getOverlap() * format.getChannels());
